...
function getErrorMessage(error?: string) {
  switch (error) {
    ...
    case "no_mix_admin":
      return "Sélectionne d’abord un admin autorisé à générer le mix Versus.";
    case "server":
...
export default async function AdminMixVersusPage({
  ...
  // ✅ Dernière session (filtrée game = VERSUS)
  ...
  const latestSession = sessionId
    ? await db.mixSession.findUnique({ where: { id: sessionId }, include: teamsInclude })
    ...
    <SiteShell>
      <div className="grid gap-5">
        ...
        {isSuccess ? (
          <div className="neon-card p-5">
            <p className="text-sm font-medium text-emerald-400">
              Équipes Versus générées avec succès.
            </p>
          </div>
        ) : null}
        ...
        {isLockSet ? (
          <div className="neon-card p-5">
            <p className="text-sm font-medium text-cyan-300">
              Admin générateur Versus mis à jour.
            </p>
          </div>
        ) : null}
        ...
        {isLockCleared ? (
          <div className="neon-card p-5">
            <p className="text-sm font-medium text-amber-300">
              Sélection du générateur Versus retirée.
            </p>
          </div>
        ) : null}
          ...
          <div className="neon-card p-4">
            <div className="flex flex-col gap-3">
              <div>
                ...
                <h3 className="mt-1 text-base font-bold text-white">
                  Invité Versus (sans compte)
                </h3>
                <p className="neon-text-muted mt-1 text-xs leading-5">
                  Ajoute un pseudo temporaire dans le pool Versus (séparé des autres jeux).
                </p>
              </div>
          ...
          {mixLock?.selectedUser ? (
            <p className="mt-4 text-sm text-white/80">
              ...
              {!canCurrentAdminGenerate
                ...
                : " Tu peux générer le mix Versus."}
            </p>
          ) : (
            <p className="mt-4 text-sm text-amber-300">
              Aucun admin n’est sélectionné pour générer le Versus.
            </p>
          )}
            ...
            {totalPoolCount === 0 ? (
              <div className="neon-card-soft p-4">
                <p className="neon-text-muted text-sm">
                  Aucun joueur disponible pour le Versus.
                </p>
              </div>
            ) : (
              <>
        ...
        <div className="neon-card p-5 md:p-6">
          ...
          <h3 className="mt-2 text-lg font-bold text-white md:text-xl">
            Ajouter un joueur inscrit au pool Versus
          </h3>
                ...
                <div key={team.id} className="neon-card-soft p-3">
                  ...
                  <div className="mt-2.5 grid gap-2">
                    {team.members.map((member) => {
                      ...
                      const badges = member.user?.badges ?? [];
