...
export default async function VersusPage({
  ...
  const user = await db.user.findUnique({
    ...
    select: { id: true, isAvailableForVersusMix: true },
  });
  ...
  const [queueUsers, queueTempPlayers, canManagePool, managingAdminName] = await Promise.all([
    db.user.findMany({
      where: { status: "ACTIVE", registrationStatus: "APPROVED", isAvailableForVersusMix: true },
    ...
    canSelfServeMix("VERSUS", sessionUser.id),
    getMixManagingAdminName("VERSUS"),
  ]);
      ...
      <div className="grid gap-4 md:gap-6">
        <div className="neon-card p-5 md:p-8">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            >
              {isInQueue ? "Prêt (Versus)" : "En attente"}
            </span>
        ...
        <div className="neon-card p-6 md:p-8">
          ...
          {teams.length === 0 ? (
            <p className="neon-text-muted mt-4 text-sm">
              Aucune équipe Versus pour le moment.
            </p>
          ) : (
            <TeamPager>
              {teams.map((team) => {
