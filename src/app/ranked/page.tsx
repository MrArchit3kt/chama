...
function getErrorMessage(error?: string) {
  switch (error) {
    case "forbidden":
      return "Rejoins d’abord la file Ranked pour pouvoir générer.";
    case "pool_forbidden":
      return "Un admin gère actuellement la file : tu ne peux pas retirer de joueur.";
    case "invalid_count":
      return "En Ranked, le mix doit être en 3v3 uniquement : il faut un nombre de joueurs divisible par 3.";
    case "locked":
...
export default async function RankedPage({
  ...
  const user = await db.user.findUnique({
    ...
    select: { id: true, isAvailableForWarzoneRankedMix: true },
  });
  ...
  const isInQueue = !!user.isAvailableForWarzoneRankedMix;

  const [queueUsers, queueTempPlayers, canManagePool] = await Promise.all([
    db.user.findMany({
      where: { status: "ACTIVE", registrationStatus: "APPROVED", isAvailableForWarzoneRankedMix: true },
    ...
    canSelfServeMix("WARZONE_RANKED", sessionUser.id),
  ]);
      ...
      <div className="grid gap-4 md:gap-6">
        <div className="neon-card p-5 md:p-8">
          ...
          <p className="neon-text-muted mt-3 max-w-3xl text-sm leading-6 md:mt-4 md:text-base md:leading-7">
            Le Ranked est strict : équipes de 3 uniquement. Rejoins la file pour
            être pris en compte, puis consulte toutes les équipes (la tienne en
            premier).
          </p>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            >
              {isInQueue ? "Prêt (Ranked)" : "En attente"}
            </span>
        ...
        <div className="neon-card p-6 md:p-8">
          ...
          <PoolList game="WARZONE_RANKED" players={poolPlayers} canManage={canManagePool} />
        </div>
        ...
        <div className="neon-card p-6 md:p-8">
          ...
          {teams.length === 0 ? (
            <p className="neon-text-muted mt-4 text-sm">
              Aucune équipe Ranked pour le moment.
            </p>
          ) : (
